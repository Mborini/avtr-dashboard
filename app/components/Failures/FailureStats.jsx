...
const exportExcel = () => {
  ...
  const rows = Object.entries(stats).map(
    ([district, data]) => {
      ...
      const field =
        statuses.PendingFieldMonitorVerification || 0;
      ...
      return {
        ...
        "في انتظار القبول":
          statuses.PendingSpValidation,

        "قيد التنفيذ":
          statuses.InProgress,

        "قيد مراجعة AVTR":
          statuses.PendingSupervisorReview,

        "انتظار التحقق الميداني":
          statuses.PendingFieldMonitorVerification,

        "تم الحل":
          statuses.Resolved,

        "تم رفض الحل":
          statuses.ResolutionRejected,
  ...
  totalRow["نسبة الإنجاز"] =
    total
      ?
      (
        (
          (
            totalRow["انتظار التحقق الميداني"] +
            totalRow["تم الحل"]
          )
          /
          total
